...
**How it is checked.** List the directory containing the audited role file; confirm the auditor file's existence at the canonical name. Parse the auditor file's YAML frontmatter; confirm `audits:` field is present and points to the actual audited role file's filename; confirm `follows_pattern:` field is present and names this spec (form: `Project Prestidigitonium v<X.Y.Z>` or equivalent). The version named is the **conformed** version — the latest release whose conformance-relevant surfaces the instance satisfies — not necessarily the repo's newest release: a pin older than HEAD is conformant when no intervening release changed a surface the instance must conform to (pin semantics in `template-auditor-role-file.md` frontmatter guidance).

> **This field list is not exhaustive of frontmatter conformance (v0.3.5).** It enumerates the fields that establish the *audit relationship*. The **governance pin block** — the versions of the artifacts the auditor itself operates under — is verified at **C-24**, not here. Read as a closed list, this check has already caused the omission C-24 exists to catch.
...
> **This check is a SECTION enumeration (v0.3.5).** "Follows the base role template" is verified here as *headings present*; it does **not** reach frontmatter fields. Frontmatter conformance lives at **C-1** (audit relationship) and **C-24** (governance pins).
...
## Criterion C-23 (Recommended) — Comms reliability provisions adopted and proven

**What is checked.** Where the project adopts the reliability provisions of `comms-spec.md` §Reliability, liveness is by heartbeat **age** (a heartbeat per role under `.comms/heartbeats/`, judged against the per-role freshness in `comms.conf`), *unread* is the order-independent processed-set (`cursors/<role-slug>.done`, advancing on action), **the full `heartbeat` command surface of the adopted `comms-spec.md` version is present** — that version's command-surface contract, *not a list copied here* (a copied list silently drifts as the surface grows; e.g. the v0.6.0 surface adds `heartbeat gate`) — and the reliability claim is backed by its falsification instrument: `heartbeat selftest` is reproducibly identical-GREEN across runs **and exercises that adopted surface in full** (selftest-GREEN therefore mechanically requires every command the adopted version defines, newly-added ones included). The suite also **injects each failure mode the adopted version's §"Reliability as a falsifiable property" enumerates** — that section's failure-mode set, *not a list copied here* (e.g. the v0.6.2 own-session-armed→fail-closed-on-absent-source and dead-wake/still-fresh-marker→armed=no cases) — so a GREEN selftest that runs a command's happy path while omitting a defined failure mode does not establish reliability.

**How it is checked.** Confirm `.comms/bin/` provides the command surface **of the adopted `comms-spec.md` version** (cross-check against that version's command-surface contract — e.g. a v0.6.0 adopter exposes `heartbeat gate`) and `comms.conf` registers the slugs. Run `heartbeat selftest` two or more times; confirm identical PASS-all across runs (the deterministic failure-injection suite) **and that the suite exercises the adopted surface in full — including commands the adopted version added** (e.g. the v0.6.0 `gate` armed-fail path). Confirm liveness is by age (a stale-and-unmarked heartbeat reads DEGRADED; a stand-down marker reads DORMANT, overriding the lagging age) and that no second process writes a role's heartbeat (the single-honest-writer property). Confirm the `selftest` **injects each failure mode named in the adopted version's §"Reliability as a falsifiable property"** (e.g. v0.6.2's absent-source→fail-closed and dead-wake/fresh-marker→armed=no), not merely that each command runs.

**Failure.** Reliability claimed without a passing `selftest`; **a command defined by the adopted `comms-spec.md` version's surface absent — or present but not exercised by the `selftest`** (a GREEN selftest that omits a defined command, e.g. the v0.6.0 `gate`, does not establish that command); liveness judged by process-presence rather than heartbeat age; a role's heartbeat written by more than one process (a masked-death double-writer); *unread* tracked by a single mutable high-water rather than the durable processed-set; **a failure mode named in the adopted `comms-spec.md` version's §"Reliability as a falsifiable property" absent from the `selftest`** (a GREEN suite that exercises the command surface but omits a defined failure mode — e.g. v0.6.2's absent-source→fail-closed — does not establish reliability).

> Note: Recommended rather than Required because the comms channel (C-22) functions with session-long watches and owner-relayed verdicts; the reliability provisions harden liveness, activation, and cross-death recovery for pairs that run unattended or at scale. A deployment may adopt the channel (C-22) without the reliability layer (C-23). Follows the C-20/C-21 precedent of landing Recommended criteria in a patch release.

## Criterion C-24 (Recommended) — Governance the auditor operates under is pinned, and compared

**What is checked.** Three conjuncts.

1. **Presence.** The auditor file's frontmatter carries a machine-readable block of
   `<governance-artifact>: <version>` pairs.
2. **Completeness.** Every governance artifact the file **binds** — named in a Normative
   Requirement, an Operational Constraint, or an **Inputs** entry as something the auditor operates
   under — has an entry. *Binding makes a rule apply; pinning makes its drift detectable.*
3. **Currency (the conjunct that makes the other two worth having).** The auditor's staleness-check
   discipline **compares** each pinned version against the live artifact at session start, and
   treats a mismatch as a reportable finding. **A pin block that is present, complete, and never
   compared is a plaque** — the record without the reader. This mirrors C-21's engineering exactly:
   C-21's value is not the `audits_version:` field but the load-time comparison it enables.

**How it is checked.** Parse the auditor file's frontmatter for the pin block; derive the set of
bound artifacts from its Normative Requirements, Operational Constraints, and Inputs; confirm every
bound artifact has an entry. For conjunct 3, confirm the file's staleness-check clause states the
comparison. The instrument for conjunct 3 SHOULD be a **version-agnostic whole-block sweep** — a
search for one specific version can only find files pinned at the value being replaced, and is blind
**by construction** to every file pinned older. (`bin/governance-pin-check` in the Aire deployment is
one such implementation — an exemplar, not a mandate.)

**Failure.** No pin block; a bound artifact with no entry; a block with no stated comparison
discipline.

**Era conditioning.** Adopters predating this criterion are **WORK, not FAIL**: the sweep rides the
auditor file's next natural delta. **But absence is never silence** — it is a finding reportable on
sight. An absent block must be recorded as absent, never read as compliance.

> **Why Recommended, and when that is revisited.** Recommended follows the C-20/C-21/C-22/C-23
> precedent and preserves the standing invariant that no Required criteria change and
> prior-conformant implementations remain conformant — the invariant that makes C-1's
> conformed-version semantics trustworthy. The omission this criterion catches propagates by
> *peer-mimicry* (a new auditor conforming to an existing auditor rather than to the template), and
> what breaks that chain is the reportable-on-sight jaw, not the severity: once absence is a
> mandatory finding, mimicry reproduces a visible flag instead of a silent gap.
> **Revisit trigger (event-shaped, not date-shaped): promotion-to-Required is reviewed at this
> pattern's next MAJOR release.**

> **Framework neutrality.** The conjuncts are stated in outcome terms. `governance:` is the
> Aire-shaped example; a host framework with a different frontmatter convention satisfies C-24 by
> recording the same facts in its own shape. What is **not** optional is that the facts be present,
> complete over bindings, and compared.
...
Recommended criteria (C-18 through C-24) are reviewed last.
...
- time: 2026-06-07
- summary: v0.2.0 — Rewritten conformance criteria for the separate-file architecture. v0.1.x's sixteen criteria (centered on §Audit-Variant section presence and structure) replaced with nineteen criteria (seventeen Required, two Recommended) centered on separate-file topology: auditor file existence + canonical naming + correct frontmatter (C-1); reciprocal `audited_by:` pointer in audited role (C-2); base-role-template structure in auditor file (C-3); authority chain + upstream governance + adversarial default + re-derivation + authority precedence + scope limits (C-4 to C-9); per-criterion clauses + refutation framing + concrete mechanisms + cross-rule obligations (C-10 to C-13); all six relational primitives through audit lens (C-14); corpus tree (C-15); audited-role load list excludes both corpus AND auditor file (C-16); absence of §Audit-Variant in audited role (C-17); corpus frontmatter discipline + post-mortem incorporation traceability (C-18, C-19 Recommended). Three-pass review procedure updated. Load-bearing criteria called out (C-1, C-2, C-4, C-6, C-10, C-11, C-16, C-17). v0.2.2 (2026-06-07) — Adds Criterion C-20 (Recommended) for explicit `audit_posture` declaration in auditor file frontmatter and body, per the template's new posture-declaration mechanism (artifact-verdict-only / continuous-monitoring / hybrid). Absence is not a Required defect because v0.2.2's template specifies a graceful default to `artifact-verdict-only`, but explicit declaration prevents downstream ambiguity. References field updated to v0.2.2 across all three sibling specs. No Required criteria changed; v0.2.0-conformant implementations remain conformant under v0.2.2. v0.2.3 (2026-06-11) — Per ASA fresh-pass review: restores numerical ordering of the Recommended criteria (C-20 had been filed before C-19 in v0.2.2) and adds C-20/C-21 to the review-procedure closing line, which still read "C-18, C-19" after C-20 landed. Adds Criterion C-21 (Recommended) for `audits_version:` interpretation pinning — the auditor declares which version of the audited role file its per-criterion interpretations target, detecting silent interpretation drift at session start rather than waiting for a full C-10 correspondence review. Adds inconclusive-density guidance to the verdict structure: verdicts resting on more than roughly a quarter inconclusive checks SHOULD grade no higher than CONFORMANT WITH NOTES. Follows the C-20 precedent of landing Recommended criteria in a patch release; no Required criteria changed; v0.2.0-conformant implementations remain conformant under v0.2.3. v0.2.5 (2026-06-11) — C-21 gains the multi-file rule-source note per the RoleSmith Auditor's first cross-project verdict: when interpretations target rule units in files other than the pinned master (module specs), the single pin's coverage is conventional rather than mechanical; auditors in that topology SHOULD enumerate targeted files in their staleness clause and treat targeted-file version changes as staleness. Multi-pin map form deferred to v0.3.0. No criteria added or changed; guidance note only. v0.2.6 (2026-06-11) — References field converted to pin-free (filenames only; versions live in each file and the CHANGELOG) per the drift catalog's intervention; version aligned to the release lockstep. No criteria changes. v0.3.0 (2026-06-11) — Adds Criterion C-22 (Recommended): comms channel established per `comms-spec.md` (auditor file carries the read-then-arm, setup, and housekeeping duties; builder's session-entry configuration carries the bootstrap pointer; tree well-formed when present, with first-boot-pending accepted). Recommended because the pattern functions with owner-relayed verdicts; the channel removes the human relay. Review-procedure closing line extended through C-22. v0.3.1 (2026-06-11) — C-1's check gains the conformed-version pin semantics (owner-ratified): `follows_pattern:` declares the version whose conformance-relevant surfaces the instance satisfies; a pin older than HEAD is conformant when no intervening release changed a conformed surface. Resolves the re-pin treadmill the drift catalog's P3 instance demonstrated (the repair wave staled its own pins by being a release). No criteria added; check interpretation refined. v0.3.2 (2026-06-15) — Adds Criterion C-23 (Recommended): comms reliability provisions adopted and proven, per `comms-spec.md` v0.5.0 §Reliability (command surface present; `heartbeat selftest` reproducibly identical-GREEN; liveness-by-age; single honest heartbeat writer). Recommended because the channel (C-22) functions without the reliability layer; C-23 applies only where that layer is adopted. Review-procedure closing line extended through C-24. Follows the C-20/C-21/C-22 precedent of landing Recommended criteria without disturbing Required ones; v0.3.x-conformant implementations remain conformant. v0.3.3 (2026-06-24) — **C-23 de-enumeration (errata for the v0.6.0 release).** The hardcoded reference command list (`heartbeat up|wait|wake|down|doctor|status|selftest`) — which omitted the v0.6.0 `gate` and would need hand-syncing on every surface growth — is replaced by a pointer to **the adopted `comms-spec.md` version's command-surface contract**, and C-23's selftest conjunct is tied to **exercise that adopted surface in full** (closing the gap whereby a v0.6.0 adopter could pass C-23 with a gate-less surface and a GREEN selftest that never exercised `gate`). A command from the adopted surface that is absent **or present-but-unexercised** is now an explicit Failure condition (previously C-23 failed on no missing command at all). One-home-per-rule restored — the surface lives once in `comms-spec.md`; C-23 points, it does not copy — retiring the drift surface itself, per the `references:` pin-free (v0.2.6) and conformed-version (C-1) precedents. Backward-conformance preserved by C-1: a v0.5.x adopter's adopted surface is v0.5.x (no `gate` owed); only v0.6.0+ adopters owe it. Surfaced by a sibling project's decorrelated auditor (a cross-project catch) and verified refute-default by the RoleSmith Auditor. No Required criteria changed; C-23 remains Recommended; v0.3.x-conformant implementations remain conformant. v0.3.4 (2026-06-25) — **C-23 failure-mode conjunct (the cross-implementation triangulation follow-on to `comms-spec.md` v0.6.2).** v0.3.3 closed the *command-surface* gap (a defined command unexercised); the four-frame triangulation surfaced the gap one level down — C-23 is command-surface-keyed, so a happy-path `selftest` that runs `gate` satisfies it while OMITTING the *behaviour* cases (absent-source→fail-closed, dead-wake→armed=no) the comms-spec v0.6.2 §"Reliability as a falsifiable property" now mandates. C-23's selftest conjunct now ALSO requires the suite **inject each failure mode the adopted `comms-spec.md` version's §falsifiable-property enumerates**, stated by **pointing to that spec section, not copying the cases** (the same de-enumeration v0.3.3 applied to the command list — one-home, self-updating as the spec adds modes) + a Failure condition for a named failure mode absent from the `selftest`. Backward-conformance preserved by C-1 (a v0.5.x adopter owes only its adopted version's modes). Surfaced by the cross-implementation triangulation (an external aire pair + a sibling DT auditor) and ruled warranted by the RoleSmith Auditor refute-default. No Required criteria changed; C-23 remains Recommended; v0.3.x-conformant implementations remain conformant. v0.3.5 (2026-07-30) — **Adds Criterion C-24 (Recommended): the governance the auditor OPERATES UNDER is pinned, and compared.** The pattern pinned the **audited role** (C-21) and never the auditor's own governance — the identical drift, one axis over, unwatched. **The gap was caused by an enumeration, not an omission of intent:** C-3 (Required) says the auditor file "follows base role template structure" and the template body points at a host base template, but C-3's check enumerates **sections**, and C-1 — the only criterion that parses frontmatter — enumerates exactly `audits:` and `follows_pattern:`. The pointer was real and the inheritance broke anyway, because **the enumeration governs, not the pointer.** Both enumerations now carry a one-line non-exhaustiveness cross-ref to C-24, so a tool built from either does not re-inherit the gap. **C-24's third conjunct is the load-bearing one:** presence and completeness without **comparison** is a plaque — the record with no reader — so the criterion requires the staleness-check discipline to compare pins against live artifacts at session start, and names a **version-agnostic whole-block sweep** as the instrument (a search for a specific version is blind by construction to every file pinned older). **Recommended, per the C-20/C-21/C-22/C-23 precedent**, preserving the standing invariant that no Required criteria change and prior-conformant implementations remain conformant; the propagation this catches is peer-mimicry, which the **reportable-on-sight jaw** breaks rather than severity — with an **event-shaped revisit trigger** (promotion-to-Required reviewed at the next MAJOR) so Recommended cannot become permanent by default. Stated in **outcome terms**: `governance:` is the Aire-shaped example, not a mandate. **Origin:** found in an Aire deployment by an operator-run diagnostic in an **un-roled session**, where three of four auditor seats carried no block while every builder did, and the provenance of one recorded its block as *"deliberately deferred to match the in-repo conformant instance"* — peer-mimicry, documented in the act. Drafted by the RoleSmith builder, **refuted by the decorrelated auditor** (whose seat the finding lands on — self-interest declared on both sides), four folds accepted incl. this criterion's comparison conjunct; owner-authorized. No Required criteria changed; v0.3.x-conformant implementations remain conformant.
