...
**What is checked.** Where the project adopts the reliability provisions of `comms-spec.md` §Reliability, liveness is by heartbeat **age** (a heartbeat per role under `.comms/heartbeats/`, judged against the per-role freshness in `comms.conf`), *unread* is the order-independent processed-set (`cursors/<role-slug>.done`, advancing on action), the reference command surface is present (`heartbeat up|wait|wake|down|doctor|status|selftest`), and the reliability claim is backed by its falsification instrument — `heartbeat selftest` is reproducibly identical-GREEN across runs.

**How it is checked.** Confirm `.comms/bin/` provides the command surface and `comms.conf` registers the slugs. Run `heartbeat selftest` two or more times; confirm identical PASS-all across runs (the deterministic failure-injection suite). Confirm liveness is by age (a stale-and-unmarked heartbeat reads DEGRADED; a stand-down marker reads DORMANT, overriding the lagging age) and that no second process writes a role's heartbeat (the single-honest-writer property).

**Failure.** Reliability claimed without a passing `selftest`; liveness judged by process-presence rather than heartbeat age; a role's heartbeat written by more than one process (a masked-death double-writer); *unread* tracked by a single mutable high-water rather than the durable processed-set.
