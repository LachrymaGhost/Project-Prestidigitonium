...
6. **Mail is not corpus.** The builder↔auditor comms tree (`.comms/`, per `comms-spec.md`) lives outside `audit-corpus/`; the builder reading its own mail never touches auditor-only material. Verdict messages MAY cite corpus entries by id, and the builder may then read only the cited entries per rule 4.
