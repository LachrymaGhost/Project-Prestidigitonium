...
**Current: v0.6.1, status: draft.** The pattern is internally consistent and operationally complete — it can be adopted today — and has run in production across multiple projects, accumulating the running-instance lessons that shaped v0.2 through v0.5. Corpus-population conventions will keep sharpening as real instances accumulate findings.
...
- **v0.6.1 (current)** — *errata:* C-23 de-enumerated — points to the adopted `comms-spec.md` version's command surface (not a copied list, which had omitted `gate`) and ties the `selftest` to exercise it; corrects the v0.6.0 "no criteria change" note. A cross-project decorrelated-auditor catch, verified refute-default.
