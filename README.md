...
| `comms-spec.md` | Builder↔auditor mail: message-per-file inboxes, immutable messages with reply-link state, auditor-owned setup + self-triggering housekeeping, and **§Reliability** — liveness-by-age, the `.done` cursor-set (no message lost), the `heartbeat` command surface (`up`/`wait`/`wake`/`down`/`doctor`/`status`/`gate`/`selftest`) with a deterministic self-test as its falsification instrument, a per-turn unread hook so a busy agent reliably notices waiting mail, and the **wake-marker (armed-listening)** — proving the agent-waking wake is armed, not just the watch alive (`heartbeat gate` = live + armed + drained), hardened in v0.6.2 with own-session fail-closed + a coded live-task check + a single-waiter guard (closing the orphan-masks-armed stall). |
...
3. **Elevated thinking tier is a complement, not a substitute** — it makes the auditor more thorough *within the same lens* (mitigating sustained-load attention failures), not less correlated. Use it on top of model diversity.
4. **Express the pairing as a self-healing standing rule, not a brittle dated name** — model names age faster than spec versions, and a preferred model can be withdrawn earlier than its announced window. State your *preferred* pairing and use it whenever accessible; when a preferred model is unavailable, **auto-defer to the most current leading model available, at the most appropriate settings**, rather than letting the pin decay into a stale name. Re-pin only when the model or the effort/method assignment materially changes. Record this rule, dated, in the auditor file's Operational Constraints.
5. **When model diversity is temporarily impossible** (only one model available — e.g. a preferred second model withdrawn), hold decorrelation by what file separation already gives you — separate context, separate file, asymmetric corpus — **plus an effort/method asymmetry**: run the auditor at a higher effort/method tier than the builder (elevated thinking tier and, where available, multi-agent adversarial orchestration), so the capability floor is held by *method* rather than by model tier. Never downgrade the builder to manufacture the asymmetry. (Worked example, as of 2026-06: the worked pairing was a distinct model per leg — including Fable 5 as the diversity leg through its announced window; when Fable 5 was withdrawn early, both legs deferred to Claude Opus 4.8 and held decorrelation by this asymmetry — the standing rule absorbed the change with no stale pin. Restore the distinct-model leg if it returns.)
...
**Current: v0.7.1, status: draft.** The pattern is internally consistent and operationally complete — it can be adopted today — and has run in production across multiple projects, accumulating the running-instance lessons that shaped v0.2 through v0.6. Corpus-population conventions will keep sharpening as real instances accumulate findings.
...
- **v0.6.0** — **armed-listening (the wake-marker):** liveness-by-age proves the *watch* alive but not the agent-waking *wake* armed (the `up ≠ armed` / `GREEN ≠ listening` seams). The armed wake now stamps a tight-freshness, session-bound marker, so **armed-listening** is as falsifiable as liveness and a quietly-died wake is *detectable* (`heartbeat gate` → `armed=no`) — closing the residual the v0.5.4 directive could only name. Adds **`heartbeat gate`** (live + armed + drained) and a per-turn boot-incomplete cue — **the wake-marker + `heartbeat gate` are built + `selftest`-proven (43 checks incl. the production enacting path) + decorrelated-auditor-verified before landing.** The SessionStart auto-bring-up is a *convenience* whose firing is verified per-deployment (floored by the per-turn surface; `selftest` does not prove it).
- **v0.6.1** — *errata:* Criterion **C-23 de-enumerated.** It had hardcoded a `heartbeat` command list that omitted the v0.6.0 `gate`; it now points to the adopted `comms-spec.md` version's command-surface contract and requires the `selftest` to **exercise** that surface in full (a missing-or-unexercised command now fails C-23) — and corrects the v0.6.0 "no criteria change" note to an additive criteria-surface change. Surfaced by a sibling project's decorrelated auditor (a cross-project catch); verified refute-default. Backward-conformant (conformed-version semantics); no Required criterion changed.
- **v0.6.2** — **armed-listening hardened** (a four-frame cross-implementation triangulation + a parallel watcher-bug investigation): own-session **fail-closed** on an absent session source, the source **pinned by property** (not named), a **coded live-task check** (the marker carries the waiter's pid), an explicit `--cross-leg` freshness-floor, and a **single-waiter (singleton) provision** closing the orphan-masks-armed stall (a `/compact`-orphaned waiter passes liveness yet masks `armed`). C-23 gains a failure-mode conjunct (the orphan-injection, auto-covered by its de-enumeration). Reference impl built + `selftest`-proven (50/0 ×N incl. the orphan-injection) + decorrelated-auditor-GREEN before landing. Auditor-role-file `follows_pattern:` pins (v0.6.1) stay conformant — only the comms-reliability surface moved.
- **v0.7.0** — **peer reconciliation (multi-leg adoptions):** `auditor-pattern-spec.md` §"Peer reconciliation" canonizes how decorrelated legs handle a **split** — independent derivation first → **debate/negotiate/triangulate, not immediate escalation** → an unresolved split surfaced to the user decision-shaped with both positions preserved → **never averaged/voted/auto-reconciled** (a CONFORMANT-vs-REFUTED split is the *decorrelation prize*, not noise) → a losing position kept as **corpus** (being wrong teaches). Applies to auditor↔auditor peer-triangulation **and** builder↔auditor forks; the user stays authoritative. Additive (single-auditor adoptions have no peer) → `follows_pattern:` pins stay conformant. Forward: a multi-auditor concurrency + verdict-reconciliation spec elaborates it.
- **v0.7.1** — **safety-class carve-out:** `auditor-pattern-spec.md` §"Peer reconciliation" point (2) gains an exception — a split on a **safety / security / legality / irreversible-harm** boundary **fails toward escalation** (surfaced to the user *immediately*, debate proceeds *in parallel*, not as a precondition), because debate-first is sound only while a slow resolution's cost is **bounded** and a safety-class split's is not. Timing exception to (2)/(3) only; **never-averaged** (4) and **losing-position-as-corpus** (5) unchanged; an ambiguous-class split is treated as safety-class (conservative default). Surfaced convergently by *both* the Claude and the cross-vendor auditor in the aire 3-party stability triangulation. Additive (narrows one class of (2)) → `follows_pattern:` pins stay conformant.
...
- **v0.5.4** — `comms-bringup-directive.md`: the ordered, gate-verified comms bring-up procedure; adds the autonomous heads-down wake-on-arrival operationalization (§4b / Gate #7) at the deployment layer.
- **v0.6.0** — armed-listening (the wake-marker): a session-bound liveness marker for the agent-waking wake, making a quietly-died wake *detectable*; `heartbeat gate` (live + armed + drained); a per-turn boot-incomplete cue — **the wake-marker + `heartbeat gate` built + `selftest`-proven + decorrelated-verified.** SessionStart auto-bring-up is a convenience (firing verified per-deployment, floored by the per-turn surface).
- **v0.6.1** — *errata:* C-23 de-enumerated — points to the adopted `comms-spec.md` version's command surface (not a copied list, which had omitted `gate`) and ties the `selftest` to exercise it; corrects the v0.6.0 "no criteria change" note. A cross-project decorrelated-auditor catch, verified refute-default.
- **v0.6.2 (current)** — armed-listening **hardened**: own-session **fail-closed** on an absent session source, the source **pinned by property** (not named), a **coded live-task check** (marker carries the waiter's pid), an explicit `--cross-leg` floor, and a **single-waiter (singleton) provision** closing the orphan-masks-armed stall. C-23 gains a failure-mode conjunct (auto-covered by its de-enumeration). A four-frame cross-implementation triangulation + a watcher-bug fix; reference impl built + `selftest`-proven (50/0 ×N) + decorrelated-verified.
