...
| `comms-spec.md` | Builder↔auditor mail: message-per-file inboxes, immutable messages with reply-link state, watch-arming, auditor-owned setup (the activation directive bootstraps the builder) and self-triggering housekeeping. |
...
- **v0.3.0 (current)** — the comms layer: `comms-spec.md`, adoption Step 9, Criterion C-22 (Recommended), comms duties in the template skeleton.
- **v0.3.x** — likely additions: `audits_version:` multi-pin map form (multi-file rule sources); project-level multi-role corpus sharing conventions; explicit cross-corpus pollination protocol; conformance criteria for the specifications themselves (self-conformance, currently reserved).
