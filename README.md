...
| `comms-spec.md` | Builder↔auditor mail: message-per-file inboxes, immutable messages with reply-link state, watch-arming, auditor-owned setup (the activation directive bootstraps the builder), self-triggering housekeeping, and the watch economy (reader cursors, watch-lifetime rule, commit-signal hooks). |
| `conformance-criteria.md` | Twenty-two criteria (seventeen Required, five Recommended) for verifying that an implementation realizes the pattern. Includes verdict structure and review procedure. |
...
- **v0.4.0** — watch economy: reader cursors, watch-lifetime rule + scheduled-wake package, commit-signal hooks, id-date rule. *v0.4.1 (current): skeleton pin maintenance.*
