...
**Status:** v2.2 — adds the autonomous-wake provisions (§4b + Gate #7) from a live heads-down-loop missed-mail incident. Derived from `comms-spec.md` (v0.5.2+); pin-free in body — versions live in the spec and the CHANGELOG.
...
| Event-wake torn down by a context reset (`/compact`, resume) and not re-armed | **Ongoing-discipline re-arm rule + Gate #7 (re-run after a reset)** |
