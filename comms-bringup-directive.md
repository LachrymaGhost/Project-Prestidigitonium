...
**Status:** v2.3 — adds **armed-listening** (the wake-marker) to §4b + the Standing health check: a quietly-died wake is now *detectable* and the bring-up gate is a continuous `heartbeat gate` (live + armed + drained). Derived from `comms-spec.md` (v0.6.0+); pin-free in body — versions live in the spec and the CHANGELOG.
...
```
heartbeat gate <slug>                                     # READY = live + armed-listening + drained (run from the leg's OWN session)
