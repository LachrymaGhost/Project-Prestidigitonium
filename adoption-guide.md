...
1. **Confirm the auditor file carries the comms duties** (the template skeleton includes them: read-then-arm discipline with the watch-lifetime rule, first-boot setup — including cursors and commit-signal hooks where the watch-economy provisions are adopted — and housekeeping with the hook presence check).
2. **Add the bootstrap pointer to the builder's session-entry configuration** (the CLAUDE.md you touched in Step 8, or equivalent): one line directing the builder to read `.comms/inbox-<builder-slug>/` newest-first at session start and arm a watch on it. Without this line, the activation directive waits in an inbox nothing opens — the directive cannot bootstrap itself.
...
- [ ] Step 8 — Project-harness routing wired:
  ...
  - [ ] Builder/auditor model pairing decided (auditor model ≥ builder model, different model preferred, elevated thinking tier when available) and recorded, dated, in the auditor file's Operational Constraints.
