...
**Bring it up correctly the first time.** `comms-bringup-directive.md` gives the ordered two-party bring-up sequence and a **seven-check verification gate** — prove the channel delivers, surfaces mail per-turn, gets read/replied, advances a cursor, and wakes a heads-down (autonomous/loop) agent, rather than declaring setup done on file-existence. It changes no mechanism; it sequences and verifies what `comms-spec.md` §Setup already mandates (and operationalizes the deployment-layer wake-on-arrival the spec leaves to the harness).
...
- **Subdir hooks silently shadow root hooks.** A directory-dispatched leg that gains its own `.claude/settings*.json` `hooks` key loses ALL inherited root hooks (winner-take-all per key). See *Strategy A — hook-surface completeness*. The failure is silent; guard it with a completeness check.
...
### Strategy A — hook-surface completeness (the shadow trap)

Strategy A is what makes this trap possible: it gives each leg its **own launch directory**, which invites each leg to gain its own `.claude/settings*.json` (e.g. a per-leg comms banner). The hazard is in how the runtime resolves settings.

**Claude Code resolves the `hooks` settings key winner-take-all *per key*.** The highest-priority scope that *defines* `hooks` wins **entirely** — there is no merge with an inherited/root `hooks` block. (Permissions merge across scopes; `hooks`, like other non-permission keys, does not — and the subdir-vs-root case is undocumented, so treat it as verified-by-observation, not by the docs.) Consequence: the instant a directory-dispatched leg's own `settings.local.json` defines a `hooks` key **for any reason**, that leg's sessions **lose every inherited root hook**. An additive-looking edit (adding one banner) is a *total* per-key inheritance replacement. It fails **silently** — the leg simply stops firing the root hooks, with no error.

**The rule.** A directory-dispatched leg's subdir `.claude/settings*.json` that defines a `hooks` key MUST carry the **complete intended hook set** — copy the root set, then extend (per-leg banner, etc.). If a leg deliberately omits a root hook, **record a deviation note** naming which hooks it omits and why. "Copy-then-extend," never "add-one."

**Delivering per-leg identity to the shared hooks.** Harness hooks that must know *which* leg they fire for (a cadence counter keyed per role, a per-leg capture path) need the leg's identity. Deliver it via the **hook command's own argument** — bake `--slug <role>` (and any home pin) into that leg's hook registration in its subdir settings, the same channel a per-leg banner arg already uses. Do **not** rely on the settings `env` block to reach hook subprocesses (undocumented — a silent-fail-on-upgrade dependency), and do **not** rely on a session-keyed self-scoping claim as the *primary* path (it dies per-session). The registration arg is static, deterministic, survives `--resume`/`--compact`, and needs **zero change to the launch command**.

**Enforcement.** The completeness rule is grep-checkable: compare each subdir's `hooks` event keys against the root's. Adopters running the aire `roles-compliance` profile get this as row **P17** (class-C: fires only for a leg with subdir hooks; asserts superset-or-deviation-note). Adopters not on that profile should add the equivalent one-line conformance check.
