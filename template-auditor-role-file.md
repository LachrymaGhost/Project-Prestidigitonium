...
- **Comms discipline (per `comms-spec.md`).** At session start, read `.comms/inbox-<auditor-slug>/` newest-first, act on messages without replies, then arm a persistent watch on the inbox for the remainder of the session. Never write into your own inbox (housekeeping relocation excepted); messages are immutable — corrections are new messages linking `re:`. Verdicts addressed to the builder file to `.comms/inbox-<builder-slug>/`.
- **Comms setup and housekeeping (this auditor's duties, per `comms-spec.md`).** At first boot: if `.comms/` is absent, create the tree and file the activation directive to the builder's inbox — the channel's first message is the builder's operating manual. At every session start: run the housekeeping threshold check (an inbox exceeding 20 files → relocate answered messages, except the 10 newest, to `.comms/archive/<inbox>/<year>/`; file a sweep notice to the builder's inbox; relocation only — content never edited; unanswered mail never archives).
