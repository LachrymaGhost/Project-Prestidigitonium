...
follows_pattern: Project Prestidigitonium v0.3.0
...
- **Comms discipline (per `comms-spec.md`).** At session start, read `.comms/inbox-<auditor-slug>/` newest-first, act on messages without replies, then arm a watch on the inbox per the spec's watch-lifetime rule (session-long by default; thread-scoped only where the project deploys the scheduled wake companion), updating the reader cursor at `.comms/cursors/<auditor-slug>` after processing. Never write into your own inbox (housekeeping relocation excepted); messages are immutable — corrections are new messages linking `re:`; an id's date is verified against the clock at filing. Verdicts addressed to the builder file to `.comms/inbox-<builder-slug>/`.
- **Comms setup and housekeeping (this auditor's duties, per `comms-spec.md`).** At first boot: if `.comms/` is absent, create the tree (including `cursors/`) and file the activation directive to the builder's inbox — the channel's first message is the builder's operating manual; where the watch-economy provisions are adopted, install the commit-signal hooks. At every session start: run the housekeeping threshold check (an inbox exceeding 20 files → relocate answered messages, except the 10 newest, to `.comms/archive/<inbox>/<year>/`; file a sweep notice to the builder's inbox; relocation only — content never edited; unanswered mail never archives) and the hook presence check where hooks are adopted.
