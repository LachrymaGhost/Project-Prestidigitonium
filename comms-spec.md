...
In-session activation is `heartbeat wait`: it polls unread (cursor-delta) on an **encoded bounded-adaptive interval** — a tight active rate, a rest idle rate, and a hard-max clamp applied to every wake. The cadence is out of the agent's hands (the silent-stall cause is encoded out, not left to judgment), and on `unread > 0` it exits, which the harness uses to **re-invoke the agent**. Detection is wired to activation, not to a log. The in-flight state is a **file marker** (survives death/compaction); the cursor advances on action only.
