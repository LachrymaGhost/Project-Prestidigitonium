...
- **Harness-level hook backstops** (machine configuration; recommended where available — a hook that surfaces the unread count each turn is structural enforcement of what rule 5 asks the model to remember; a wake-on-arrival hook is the only thing that closes the live-interactive-session wake the cross-death ladder leaves to the harness).
